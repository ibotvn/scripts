...
function HTTF_Ezreal:ScriptMsg(msg)
  print("<font color=\"#daa520\"><b>[Truy cập vào http://ibotvn.com Để để nện nhé]:</b></font> <font color=\"#FFFFFF\">"..msg.."</font>")
end
...
if VersionData then
  ...
  if ServerVersion then
    ...
    if tonumber(Version) < ServerVersion then
      HTTF_Ezreal:ScriptMsg("Phiên bản mới có sẵn: v"..VersionData)
      HTTF_Ezreal:ScriptMsg("Đang cập nhật, xin vui lòng không nhấn F9.")
      DelayAction(function() DownloadFile(UpdateURL, ScriptFilePath, function () HTTF_Ezreal:ScriptMsg("cập nhật thành công .: v"..Version.." => v"..VersionData..", Press F9 twice to load the updated version.") end) end, 3)
      return
    else
      HTTF_Ezreal:ScriptMsg("Bạn đã có phiên bản mới nhất: v"..Version)
    end
    
  else
    HTTF_Ezreal:ScriptMsg("Lỗi tải phiên bản máy chủ.")
  end
  
else
  HTTF_Ezreal:ScriptMsg("Lỗi tải thông tin phiên bản.")
end
...
function HTTF_Ezreal:Menu()
  ...
  self.Menu = scriptConfig("Ken Ezreal", "Ken Ezreal")
  ...
  self.Menu:addSubMenu("Cai dat Clear", "Clear")
    ...
    self.Menu.Clear:addSubMenu("Cai dat Jungle Clear", "JFarm")
  ...
  self.Menu:addSubMenu("Auto Skill Die Muc Tieu", "KillSteal")
  ...
  self.Menu:addSubMenu("Cai dat hien thi Skill", "Draw")
...
function HTTF_Ezreal:Orbwalk()
  ...
  elseif _G.NebelwolfisOrbWalkerLoaded then
    self.NOWLoaded = true
    self:ScriptMsg("Tìm thấy Orb Walk Nebelwolfi của.")
